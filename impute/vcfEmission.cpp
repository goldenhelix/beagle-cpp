...
int BitSetGTSharedData::allele1(int sample) const {
  return isMissing1Data.testBit(sample) ? -1
    : (bitsPerAllele == 1 ? allele1Data.testBit(sample) : alleleFromBits(allele1Data, sample));
}

int BitSetGTSharedData::allele2(int sample) const {
  return isMissing2Data.testBit(sample) ? -1
    : (bitsPerAllele == 1 ? allele2Data.testBit(sample) : alleleFromBits(allele2Data, sample));
}
  ...
  bool isMiss2 = isMissing2Data.testBit(sample);
        ...
        if (isMiss1  &&  isMiss2) {
            ...
            if (isPh) {
                ...
                bool isConsistent = (obsA1==a1 && obsA2==a2)
                        || (obsA1==a2 && obsA2==a1);
...
void BitSetRefGT::setIdInfo(int chromIndex, int pos, CString id)
{
...
void BitSetGT::setIdInfo(int chromIndex, int pos, CString id)
{
  _d->chromIndex = chromIndex;
  _d->pos = pos;
  _d->id = id;
