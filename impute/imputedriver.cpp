...
//// QList<HapPair> ImputeDriver::phase(const CurrentData &cd) { etc.; }

//// QList<HapPair> ImputeDriver::initialHaps(const CurrentData &cd) { etc.; }

void ImputeDriver::sample(Dag &dag, SplicedGL &gl, int seed,
                          bool markersAreReversed, int nSamplesPerIndividual,
                          QList<HapPair> &sampledHaps, int nThreads, bool lowmem)
{
  SingleBaum baum(dag, gl, seed, nSamplesPerIndividual, lowmem);
