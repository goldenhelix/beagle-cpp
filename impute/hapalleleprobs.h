...
public:
   ...
   */
  ConstrainedAlleleProbs(const SampleHapPairs &shp, const QList<HapAlleleProbs> &alProbs,
                         const QList<int> &indexMap);
   ...
   */
  ConstrainedAlleleProbs(const SampleHapPairs &sampleHapPairs);
...
/**
 * Class {@code ConstrainedGLAlleleProbs} represents per-haplotype
 * allele probabilities for a list of samples. The object's
 * GLSampleHapPairs instance is always used to obtain data for any
 * given marker.
 *
 * "All instances of {@code [ConstrainedGL]AlleleProbs} are required to
 * be immutable."
 */
...
public:
  ...
  /**
   * Constructs a new {@code ConstrainedGLAlleleProbs} instance that
   * merely wraps the specified {@code GLSampleHapPairs} object.  The
   * alleles in the specified {@code GLSampleHapPairs} instance will
   * have probability 1. The {@code SampleHapPairs} object is always
   * used to obtain the data.
   *
   * @param sampleHapPairs the sample haplotype pairs that will
   * be wrapped by {@code this}
   */
  ConstrainedGLAlleleProbs(const GLSampleHapPairs &sampleHapPairs);

  /**
   * Returns {@code false} all the time, because the specified marker
   * is always present in the (target) input data.  @param marker a
   * marker index
   */
  bool isImputed(int marker) const { return false; }
